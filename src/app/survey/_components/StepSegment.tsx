...
type Props = {
  ...
  padX?: number; // 좌우 패딩(px). 기본 20 → px-20
  gap?: number; // 바 사이 간격(px). 기본 8
  stroke?: number; // 선 두께(px). 기본 4
  color?: string; // 채워진 바 색상. 기본 #FF4F14
  emptyColor?: string; // 미채움 바 색상. 기본 rgba(0,0,0,0.12)
  className?: string;
};
        ...
        {Array.from({ length: total }).map((_, i) => (
          <div
            ...
            style={{
              ...
              borderColor: i <= active ? color : emptyColor,
              opacity: 1, // 명시적으로 1
            }}
            aria-hidden
          />
